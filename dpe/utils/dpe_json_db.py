...
def dpe_json_to_db (data: str, ademe: str, xml_path = None, extended=True, delete_if_exist=True):
    """
    Importe les données DPE depuis un fichier JSON (et non plus XML).
    """

    print(f"Import ademe {ademe}")
    ...
    dpe = Dpe.objects.filter(identifiant_dpe=ademe).first()
    if dpe:
