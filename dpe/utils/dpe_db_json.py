# dpe/utils/import_dpe_xml.py
import hashlib
import xml.etree.ElementTree as ET
from dpe.models import *
from django.utils.dateparse import parse_date
import json
from dpe.utils.dpe import date_json, to_dpe_json
...
def dpe_db_to_json (ademe, extended=True):
    ...
    infos = DpeInfos.objects.filter(dpe_id=dpe).first()
    
    administratif = (
        DpeAdministratif.objects
        .select_related('dpe_id')  # pas forcément utile ici car c'est OneToOneField sur Dpe
        .filter(dpe_id=dpe)
        .first()
    )
    ...
    if administratif:
        dpe_data['administratif'] = {
            ...
            'diagnostiqueur' : {
                'usr_logiciel_id' : infos.usr_logiciel_id,
                'version_logiciel' : infos.version_logiciel ,
                'version_moteur_calcul' : infos.version_moteur_calcul ,
                'nom_diagnostiqueur'  : infos.nom_diagnostiqueur ,
                'prenom_diagnostiqueur'  : infos.prenom_diagnostiqueur ,
                'mail_diagnostiqueur' : infos.mail_diagnostiqueur ,
                'telephone_diagnostiqueur' : infos.telephone_diagnostiqueur ,
                'adresse_diagnostiqueur'  : infos.adresse_diagnostiqueur ,
                'entreprise_diagnostiqueur'  : infos.entreprise_diagnostiqueur ,
                'numero_certification_diagnostiqueur' : infos.numero_certification_diagnostiqueur ,
                'organisme_certificateur'  : infos.organisme_certificateur ,
            },
        }
        
    geolocalisation = DpeGeolocalisation.objects.filter(administratif=administratif).first() 
    if geolocalisation:
        dpe_data['administratif']['geolocalisation'] = {
                'invar_logement' : infos.invar_logement,
                'numero_fiscal_local' : infos.numero_fiscal_local,
                'id_batiment_rnb' : geolocalisation.id_batiment_rnb,
                'rpls_log_id' : geolocalisation.rpls_log_id,
                'rpls_org_id' : infos.rpls_org_id,
                'idpar' : geolocalisation.idpar,
                'immatriculation_copropriete' : geolocalisation.immatriculation_copropriete,
            }
    
        adresse_bien = geolocalisation.adresse_bien
        if adresse_bien:
    ...
    for element in elements:
        ...
        masques = DpeBaieVitreeMasque.objects.filter(baie_vitree_id=element.id)
        masque_list = []
