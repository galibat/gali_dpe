...
- Import automatique de DPE au format XML (avec validation XSD).
- Stockage structuré en base relationnelle.
- Analyse des DPE selon un jeu de règles pondérées (précision, incohérence, fiabilité).
- Interface Django avec authentification.
- Recherche full-text par adresse ou numéro ADEME.
- Affichage cartographique des diagnostics (Leaflet + GeoJSON).
- API sécurisée pour exploitation externe (OpenAPI + tokens).
